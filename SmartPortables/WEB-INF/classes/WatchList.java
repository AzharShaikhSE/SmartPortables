...
public class WatchList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		HashMap<Integer, ProductBean> watchhm = new HashMap<Integer, ProductBean>();
		ProductBean product;
		...
		for (Integer prodId: producthm.keySet()) {
			...
			if(product.getCategory().equalsIgnoreCase("SmartWatch")){

				if(brandName==null){
					watchhm.put(product.getId(), product);
					name = "";
				}else{
					...
					else if(brandName.equals("fitbit") && product.getBrand().equalsIgnoreCase("fitbit")){
						watchhm.put(product.getId(), product);
						name = product.getBrand();
					}
		...
		int i = 1; int size= watchhm.size();

		for (Integer phoneId: watchhm.keySet()) {
			product = watchhm.get(phoneId);
