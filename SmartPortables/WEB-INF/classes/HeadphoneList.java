...
public class HeadphoneList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		HashMap<Integer, ProductBean> headphonehm = new HashMap<Integer, ProductBean>();
		ProductBean product;
		...
		for (Integer prodId: producthm.keySet()) {
			...
			if(product.getCategory().equalsIgnoreCase("Headphones")){

				if(brandName==null){
					headphonehm.put(product.getId(), product);
					name = "";
				}else{
					if(brandName.equals("skullcandy") && product.getBrand().equalsIgnoreCase("skullcandy")){
						headphonehm.put(product.getId(), product);
						name = product.getBrand();
					}
					else if(brandName.equals("sony") && product.getBrand().equalsIgnoreCase("sony")){
						headphonehm.put(product.getId(), product);
						name = product.getBrand();
					}
					else if(brandName.equals("panasonic") && product.getBrand().equalsIgnoreCase("panasonic")){
						headphonehm.put(product.getId(), product);
						name = product.getBrand();
					}
				}
		...
		int i = 1; int size= headphonehm.size();

		for (Integer headphoneId: headphonehm.keySet()) {
			product = headphonehm.get(headphoneId);
