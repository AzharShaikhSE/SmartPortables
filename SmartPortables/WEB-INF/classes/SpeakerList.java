...
public class SpeakerList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		HashMap<Integer, ProductBean> speakerhm = new HashMap<Integer, ProductBean>();
		ProductBean product;
		...
		for (Integer prodId: producthm.keySet()) {
			...
			if(product.getCategory().equalsIgnoreCase("Speakers")){

				if(brandName==null){
					speakerhm.put(product.getId(), product);
					name = "";
				}else{
					if(brandName.equals("bose") && product.getBrand().equalsIgnoreCase("Bose")){
						speakerhm.put(product.getId(), product);
						name = product.getBrand();
					}
					else if(brandName.equals("doss") && product.getBrand().equalsIgnoreCase("Doss")){
						speakerhm.put(product.getId(), product);
						name = product.getBrand();
					}
					else if(brandName.equals("logitech") && product.getBrand().equalsIgnoreCase("Logitech")){
						speakerhm.put(product.getId(), product);
						name = product.getBrand();
					}
				}
		...
		int i = 1;
		int size= speakerhm.size();

		for (Integer speakerId: speakerhm.keySet()) {
			product = speakerhm.get(speakerId);
